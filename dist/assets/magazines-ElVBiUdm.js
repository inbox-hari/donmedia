import"./FlipbookReader-CtuZ8Z3s.js";import"./modulepreload-polyfill-Da00irRU.js";import"./hero-stars-LBTYKB-4.js";import"./reader-modal-entry-Cos9IzTU.js";import{t as e}from"./supabase-client-BGZi-Iox.js";var t=document.getElementById(`magazine-list`);function n(e){return String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function r(t){if(console.log(`File path:`,t),!t)return``;if(/^(https?:|data:|blob:)/i.test(t)||t.includes(`/storage/v1/object/public/`))return t;let n=String(t).split(`/`);if(n.length<2)return console.warn(`Unknown storage path format:`,t),t;let r=n.shift(),i=n.join(`/`),{data:a}=e.storage.from(r).getPublicUrl(i);return console.log(`Public URL:`,a?.publicUrl),a?.publicUrl||t}async function i(){if(!t)return;t.innerHTML=`
        <div style="grid-column:1/-1;text-align:center;padding:3rem;color:#94a3b8">
          <i class="fas fa-circle-notch fa-spin" style="font-size:1.8rem;margin-bottom:.6rem;display:block"></i>
          Loading magazines…
        </div>`;let{data:i,error:a}=await e.from(`magazines`).select(`*`).order(`created_at`,{ascending:!1});if(a){t.innerHTML=`<div style="grid-column:1/-1;text-align:center;padding:3rem;color:#dc2626">Failed to load magazines. Please try again later.</div>`,console.error(`Fetch magazines error:`,a);return}if(console.log(`Books from DB:`,i),!i||!i.length){t.innerHTML=`
          <div style="grid-column:1/-1;text-align:center;padding:3rem;color:#94a3b8">
            <i class="fas fa-newspaper" style="font-size:2.5rem;opacity:.3;display:block;margin-bottom:.8rem"></i>
        ...
        <div class="product-card" data-book="${n(t)}" role="button" tabindex="0"
          ...
          <div class="product-img-container">
            ${e.cover_url?`<img src="${n(e.cover_url)}" alt="${n(e.title)} cover" loading="lazy" width="300" height="400" />`:`<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#f0f0f0;color:#94a3b8;font-size:2rem"><i class="fas fa-newspaper"></i></div>`}
          </div>
          <div class="product-info">
            <div class="product-title">${n(e.title)}</div>
            ${e.description?`<div class="product-author" style="font-size:.88rem;color:#555;margin-bottom:.5rem">${n(e.description)}</div>`:``}
            <a href="reader.html?book=${encodeURIComponent(t||``)}" class="read-btn"
               style="display:inline-flex;align-items:center;gap:.35rem;margin-top:.4rem;padding:.35rem .8rem;
                      background:#1e3799;color:#fff;border-radius:8px;font-size:.82rem;font-weight:700;
